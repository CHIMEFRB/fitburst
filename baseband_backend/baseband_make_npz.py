...
def make_fitburst_input(path : str, peaks : list = None,
    DM : float = None, fit_DM : bool = True,
    ...
    ref_freq : float = 600., freq_range : tuple = None, 
    time_range : tuple = None, output : str = None) -> None:
    """
    ...
    data, freq_id, freq, power, DM, downsampling_factor, t_res, mus, valid_channels = baseband.baseband_processing(path,
                   downsample2 = downsample2, freq_range = freq_range,
                   downsample = downsample, DM = DM, fit_DM = fit_DM,
                   peaks = peaks, fill_missing_time = fill_missing_time, save = save, spectrum_lim = spectrum_lim, 
                   time_range = time_range, diagnostic_plots=diagnostic_plots)
    ...
    "burst_width"          : [0.01] * len(mus), # a list containing the temporal widths, in seconds
    "dm"                   : [DM] * len(mus), # a list containing the dispersion measures (DM), in parsec per cubic centimeter
    "dm_index"             : [-2] * len(mus), # a list containing the exponents of frequency dependence in DM delay
    "ref_freq"             : [ref_freq] * len(mus), # a list containing the reference frequencies for arrival-time and power-law parameter estimates, in MHz (held fixed)
    "scattering_index"     : [-4] * len(mus), # a list containing the exponents of frequency dependence in scatter-broadening
    "scattering_timescale" : [0] * len(mus), # a list containing the scattering timescales, in seconds
    "spectral_index"       : [0] * len(mus), # a list containing the power-law spectral indices
    "spectral_running"     : [0] * len(mus) # a list containing the power-law spectral running
    }
